...
4. **Create Storage Location**:
   ...
   **Alternative: MinIO Setup Using Docker**

   For scenarios where you prefer running MinIO as a standalone Docker container (e.g., on a dedicated backup server or external to the cluster):
   ...
   # Create backup bucket using MinIO client
   docker exec -it minio mc alias set myminio http://localhost:9000 minioadmin minioadmin123
   docker exec -it minio mc admin user add myminio rancheruser rancherpass
   docker exec -it minio mc admin policy attach myminio writeonly --user rancheruser
   docker exec -it minio mc mb myminio/rancher-backups

   # Or if running on the same host, use localhost
   docker run --rm -it \
     --network host \
     --entrypoint=/bin/sh \
     minio/mc \
     -c "mc alias set myminio http://localhost:9000 minio-root-user minio-root-password && mc mb myminio/rke2-backups"
   ```
   ...
   **Docker Compose Alternative:**
   ...
   # Create bucket
   docker-compose exec minio mc alias set myminio http://localhost:9000 minio-root-user minio-root-password
   docker-compose exec minio mc mb myminio/rke2-backups
   ```

   **Access MinIO Console:**
   - Web UI: http://localhost:9001
   - API Endpoint: http://localhost:9000
   - Use the root credentials to log in

   **For Rancher Backup Configuration:**
   When configuring the storage location in Rancher, use:
   - **Endpoint**: `minio.example.com:9000` (replace with your Docker host IP/hostname)
   - **Bucket Name**: `rke2-backups`
   - **Credentials**: Use the MinIO root user and password
...
# Verify snapshot
rke2 etcd-snapshot status /var/lib/rancher/rke2/server/db/snapshots/manual-snapshot.db
```
...
sudo rke2 server --cluster-reset --etcd-s3=false --cluster-reset-restore-path=/var/lib/rancher/rke2/server/db/snapshots/snap-with-umdemo1.db-master-1765379268
   ```
...
rke2 etcd-snapshot save --name ${BACKUP_DIR}/etcd-snapshot-${DATE}.db
